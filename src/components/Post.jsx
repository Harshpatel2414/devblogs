...
const Post = ({ author, createdAt, title, description, image, _id }) => {
    ...
    return (
        <div className='bg-rose-50 dark:bg-zinc-800 w-full items-center md:items-start p-5 flex flex-col'>
            ...
            <div className='flex flex-col md:flex-row gap-2 h-full w-full pt-5'>
                <div className='w-full flex flex-col justify-between'>
                    <div className='flex flex-col w-full'>
                        <h1 className='text-3xl capitalize font-semibold'>{title}</h1>
                        <div className='text-lg text-wrap text-zinc-600 dark:text-zinc-500 ' dangerouslySetInnerHTML={{ __html: description }}></div>
                    </div>
                    <Link className='underline capitalize text-red-400 mt-2' href={`/blog/${_id}`}>read more</Link>
                </div>
